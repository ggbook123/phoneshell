...
## 反向代理部署（本机 19090，对外 9090）
场景：外部访问 `http://43.140.37.188:9090/panel`，Nginx 反代到本机 `19090`。

关键点：
- PhoneShell 仅监听内网端口：`--port 19090` 或 `PHONESHELL_PORT=19090`
- `publicHost` 必须带外部端口：`43.140.37.188:9090`
- Nginx 负责对外监听 `9090` 并代理 `/ws/`、`/panel`、`/api/`
...
- `PHONESHELL_MODE`
- `PHONESHELL_NAME`
- `PHONESHELL_PUBLIC_HOST`
- `PHONESHELL_PORT`
- `PHONESHELL_RELAY_URL`
- `PHONESHELL_RELAY_TOKEN`
- `PHONESHELL_GROUP_SECRET`
...
示例：
- `curl http://127.0.0.1:19090/ws/healthz`
- `curl -H "Authorization: Bearer <token>" http://127.0.0.1:19090/ws/status`
