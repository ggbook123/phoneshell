...
import type { DeviceListMessage, SessionInfo, SessionListMessage, TerminalOpenedMessage, TerminalOutputMessage, TerminalClosedMessage } from './protocol/messages.js';
...
function resolvePort(configPath: string): number {
  const cfg = loadConfigJson(configPath);
  const port = cfg?.port;
...
function readGroupSecret(baseDir: string): string {
  try {
    const groupPath = path.join(baseDir, 'data', 'group.json');
    if (!fs.existsSync(groupPath)) return '';
    const json = JSON.parse(fs.readFileSync(groupPath, 'utf-8')) as { groupSecret?: string };
...
async function fetchStandaloneInfo(httpBase: string): Promise<StandaloneInfo | null> {
  try {
    const url = `${httpBase}/api/standalone/info`;
    const info = await httpGetJson(url) as StandaloneInfo;
    if (info && info.deviceId) return info;
  } catch {
    // ignore
  }
  return null;
...
async function promptSelection<T>(items: T[], render: (item: T, index: number) => string, prompt: string): Promise<T | null> {
...
async function runLocal(options: LocalOptions): Promise<void> {
  const configPath = options.configPath || DEFAULT_CONFIG_PATH;
  const baseDir = resolveBaseDirectory(configPath);
  ...
  if (!token) token = readGroupSecret(baseDir);
  if (!token) {
  ...
  if (!activeDeviceId) {
    ...
    const deviceList = await waitForMessage<DeviceListMessage>(bus, 'device.list', 5000);
    ...
    if (deviceList.devices.length === 1) {
      ...
      const selected = await promptSelection(
        deviceList.devices,
        (d) => `${d.displayName} (${d.os}) - ${d.deviceId}`,
        'Select device: '
      );
  ...
  if (autoAttachAllowed && sessions.length > 0) {
    ...
    const opened = await waitForMessage<TerminalOpenedMessage>(bus, 'terminal.opened', 5000);
    if (!opened) {
...
async function fetchSessions(ws: WebSocket, bus: EventEmitter, deviceId: string): Promise<SessionInfo[]> {
  ws.send(serialize({ type: 'session.list.request', deviceId }));
  const list = await waitForMessage<SessionListMessage>(bus, 'session.list', 5000);
  if (!list || list.deviceId !== deviceId) return [];
  return Array.isArray(list.sessions) ? list.sessions : [];
}

function attachToSession(ws: WebSocket, bus: EventEmitter, deviceId: string, sessionId: string): void {
  const resize = () => {
  ...
  bus.on('terminal.output', (msg: TerminalOutputMessage) => {
    if (msg.deviceId !== deviceId || msg.sessionId !== sessionId) return;
    process.stdout.write(msg.data);
  });

  bus.on('terminal.closed', (msg: TerminalClosedMessage) => {
    if (msg.deviceId !== deviceId || msg.sessionId !== sessionId) return;
    process.stdout.write('\n[psh] session closed\n');
    process.exit(0);
  });
...
function waitForMessage<T>(bus: EventEmitter, type: string, timeoutMs: number): Promise<T | null> {
  return new Promise((resolve) => {
    const handler = (msg: T) => {
      clearTimeout(timer);
      bus.off(type, handler as unknown as (...args: any[]) => void);
      resolve(msg);
    };
    const timer = setTimeout(() => {
      bus.off(type, handler as unknown as (...args: any[]) => void);
    ...
    timer.unref?.();
    bus.on(type, handler as unknown as (...args: any[]) => void);
  });
}
...
main().catch((err) => {
  console.error(`[psh] fatal: ${(err as Error).message}`);
  process.exit(1);
});
