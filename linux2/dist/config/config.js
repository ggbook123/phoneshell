...
function applyEnvVars(config) {
    ...
    if (relayToken)
        config.relayAuthToken = relayToken;
...
function applyCliArgs(config, args) {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        switch (arg) {
            ...
            case '--relay-token':
                if (i + 1 < args.length)
                    config.relayAuthToken = args[++i];
                break;
            case '--group-secret':
                if (i + 1 < args.length)
                    config.groupSecret = args[++i];
                break;
            case '--public-host':
                if (i + 1 < args.length)
                    config.publicHost = args[++i];
                break;
            case '--mode': {
                if (i + 1 < args.length) {
            ...
            case '--disable-web-panel':
                config.modules.webPanel = false;
                break;
            case '--enable-ai-chat':
                config.modules.aiChat = true;
                break;
            case '--disable-ai-chat':
                config.modules.aiChat = false;
                break;
        }
    }
    return configPath;
...
function autoDetectMode(config) {
...
export function loadConfig(args) {
    ...
    if (fileConfig.displayName !== undefined)
        config.displayName = fileConfig.displayName;
    if (fileConfig.publicHost !== undefined)
    ...
    if (fileConfig.relayAuthToken !== undefined)
        config.relayAuthToken = fileConfig.relayAuthToken;
    if (fileConfig.groupSecret !== undefined)
        config.groupSecret = fileConfig.groupSecret;
    if (fileConfig.defaultCols !== undefined)
        config.defaultCols = fileConfig.defaultCols;
    if (fileConfig.defaultRows !== undefined)
